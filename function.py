...
**function.py** 是一个Python模块，它主要包含了一系列函数，这些函数用于调整和处理交易策略中的参数。该模块的功能主要集中在以下几个方面：

1. **调整随机休眠时间区间**：
   - 根据市场价格变动调整随机休眠时间区间，以响应市场的变化，减少交易频率或快速响应市场。

2. **初始化交易策略参数**：
   - 初始化交易策略中使用的动态参数，如价格变动区间计数器、随机时间区间界限、开仓上下限等。

3. **更新价格变动区间计数器**：
   - 根据当前价格变动更新不同价格变动区间的计数器，这些计数器用于跟踪市场在特定价格变动区间内的行为。

4. **调整开仓上下限**：
   - 基于当前价格与上一次价格的比较结果，调整开仓上下限，以降低交易频率和风险。

5. **保存和加载交易策略参数**：
   - 保存当前的动态参数到文件，以及从文件中加载动态参数，以便在程序启动时能够加载这些参数，保持策略的连续性和状态的持久性。

6. **执行止盈操作**：
   - 在触发止盈条件时，执行止盈操作，并在操作后重新初始化相关参数。
...
def modulate_randomtime(random_start: int, random_end: int, before_price: float, current_price: float) -> (int, int):
    ...
    if last_p_p >= 0.01:
        ...
        # 变化很慢，增加休眠时间以减少交易频率
        random_start, random_end = random_start + 19, random_end + 38
...
def statistics_profit(o: MyOkx, trade_type: int, profit: float) -> float:
    ...
    if trade_type in[3,2,-2]:  # 说明交易类型是止损平仓
